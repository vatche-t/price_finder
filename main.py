...
logger.add(sink='price_scraping_logs.log', rotation="50 MB")
...
while True:
    try:
        process = subprocess.run([f'{parent_path}/.venv/bin/python3.10', f'{parent_path}/pipeline.py'],
                                 text=True, stdout=sys.stdout, stderr=sys.stderr, timeout=max_process_life_time)
